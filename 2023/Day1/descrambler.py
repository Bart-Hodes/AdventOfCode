...
def convert_word_to_digit(match):
    ...
    return str(mapping.get(word, word))   # Return the digit if in mapping, else the word itself
...
for line in f:
    ...
    line = re.sub("(?=(one|two|three|four|five|six|seven|eight|nine))", convert_word_to_digit, line, flags=re.IGNORECASE)
